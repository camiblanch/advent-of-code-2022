...
const part2 = () => {
	...
	const fileSystemDiskSpace = 70000000;
	const updateSpaceNeeded = 30000000;

	const unusedSpace = fileSystemDiskSpace - fileSystem.dirSize;
	const minNeededToDelete = updateSpaceNeeded - unusedSpace;

	let subDirToDelete = fileSystem;

	const findDirToDelete = (dir) => {
		dir.subDirs.forEach(subDir => {
			if (subDir.dirSize >= minNeededToDelete) {
				if (subDir.dirSize < subDirToDelete.dirSize) {
					subDirToDelete = subDir;
				}
				findDirToDelete(subDir);
			}
		});
	};

	findDirToDelete(fileSystem);
	console.log("Delete dir: ", subDirToDelete.dirName);
	console.log("to delete: ", subDirToDelete.dirSize);
};
